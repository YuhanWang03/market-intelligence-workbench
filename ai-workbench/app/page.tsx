...
import { apiJson, askAgentV2, authHeaders, getAgentV2Job, type PageContext, type PageSelection, type AgentV2Evidence, type AgentV2Job, type AgentV2Response, type AgentV2SubAgent, type AgentV2TraceStep } from './lib/api';
...
export default function Home() {
  ...
  }, [allowAgentWeb, chatBusy, chatContext, chatMode, section, researchTool, labTool, researchTicker, researchResult, researchBusy, isDemo, dataError, pageSelection]);
  ...
  return <main className="app-shell"><header className="app-header"><div className="brand"><span className="brand-mark">HF</span><span>AI Hedge Fund</span></div><nav className="main-nav" aria-label="主导航">{([['core', '盯盘'], ['research', '研究'], ['lab', '实验室'], ['cost', '花费']] as const).map(([id, label]) => <button key={id} className={section === id ? 'active' : ''} onClick={() => changeSection(id)}>{label}</button>)}</nav><time className="header-clock eastern" aria-label="本机时间" suppressHydrationWarning><span>本机</span>{currentTime || '---- -- -- --:--:--'}</time><time className="header-clock eastern" aria-label="美东时间" suppressHydrationWarning><span>美东</span>{easternTime || '---- -- -- --:--:--'}</time><button className="header-status" onClick={() => setShowConnection(true)}><span className={`status-dot ${isDemo ? 'demo' : ''}`}/><span>{isDemo ? '配置连接' : '实时数据已连接'}</span><span className="header-time">设置</span></button></header>
    <div className="product-layout"><section className={`main-workspace section-${section} ${section === 'research' || section === 'lab' ? 'with-sidebar' : ''}`}>{(section === 'research' || section === 'lab') && <aside className="tool-sidebar"><div className="sidebar-label">{section === 'research' ? '研究工具' : '实验工具'}</div>{(section === 'research' ? researchMenu : labMenu).map(item => { const active = section === 'research' ? researchTool === item.id : labTool === item.id; return <button key={item.id} className={active ? 'active' : ''} onClick={() => section === 'research' ? setResearchTool(item.id as ResearchTool) : setLabTool(item.id as LabTool)}><span>{item.icon}</span>{item.label}</button> })}</aside>}<div className="workspace-content">{section === 'core' && <CorePage portfolio={portfolio} risk={risk} tape={tape} history={history} activity={activity} activityWarning={activityWarning} monitoringUniverse={monitoringUniverse} alertFilter={alertFilter} setAlertFilter={setAlertFilter} refresh={() => void refreshData()} refreshing={isRefreshing} lastUpdated={lastUpdated} dataError={dataError} watchlist={watchlist} priceAlerts={priceAlerts} addMonitoringTicker={addMonitoringTicker} removeMonitoringTicker={removeMonitoringTicker} addWatchlist={addWatchlist} removeWatchlist={removeWatchlist} addPriceAlert={addPriceAlert} removePriceAlert={removePriceAlert}/>} {section === 'research' && <ResearchPage tool={researchTool} ticker={researchTicker} setTicker={setResearchTicker} ask={askWithContext} run={runResearch} result={researchResult} busy={researchBusy}/>} {section === 'lab' && <LabPage tool={labTool} selectTool={setLabTool} ask={askWithContext} askNow={askAndSend}/>} {section === 'cost' && <CostPage report={costReport} loading={costLoading} error={costError} refresh={refreshCosts}/>}</div></section>
      <aside className="chat-panel" aria-label="AI 投资助理">
        ...
        <div className="quick-actions">{(chatMode === 'agent_v3' ? ['什么是自由现金流？', '查询 NVDA 最近收盘价和来源', '查阅 NVIDIA 最新 10-K 的供应链风险原文'] : ['比较 NVDA 和 AMD 的风险', '分析 AAPL 的估值', '回测 NVDA 动量策略']).map((suggestion, index) => <button type="button" key={suggestion} onClick={() => setChatInput(suggestion)}>{chatMode === 'agent_v3' ? ['知识问答', '行情来源', '财报原文'][index] : ['风险比较', '估值研究', '策略回测'][index]}</button>)}</div>
        <form className="chat-composer" onSubmit={handleChat}><textarea aria-label="向 AI 提问" value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); void sendMessage(chatInput) } }} placeholder={chatMode === 'agent_v3' ? '让 Agent V3 查询行情、研究或查阅原文…' : '让 Agent V2 研究、比较或运行实验…'} rows={2}/><button type="submit" disabled={chatBusy || !chatInput.trim()}>发送</button></form>
      </aside></div>
