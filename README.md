...
> [!WARNING]
> **Agent V2 和 Agent V3 目前都仍在持续调整中，尚未完成正式测评。** 仓库已有单元测试、契约测试、离线回归样例、评测脚手架和部分验收脚本，但这些结果只能证明特定代码路径可运行，不能视为完整的模型质量评估，也不能据此得出 V2 与 V3 的优劣结论。后续仍需在统一模型、统一数据快照、统一问题集、统一工具预算和人工盲审条件下完成正式评测。在正式报告发布前，请把两个 Agent 都视为实验性研究助手，不应用于无人监督的真实交易决策。
...
公开演示站点：**[https://market-workbench.duckdns.org/](https://market-workbench.duckdns.org/)**

站点使用 HTTPS，并在进入工作台前提供两种身份：

- **访客模式**：点击“以访客身份查看”，无需账号。可以浏览所有者最后发布的只读快照，不会调用模型、搜索、财务数据、券商或其他实时接口，也不会产生新的 API 费用。
- **所有者模式**：输入服务器配置的用户名和密码。可以刷新行情、管理监控、运行研究和实验，并调用 Agent V2 / V3。所有者凭据不随仓库公开，也不会写入浏览器 `localStorage`。

### 页面导航

| 页面 | 主要用途 | 访客模式 |
| --- | --- | --- |
| 盯盘 | 查看组合净值、持仓、实时异常、价格提醒、风险和行情图表 | 可查看最后发布的快照；刷新与监控管理禁用 |
| 研究 | 查看个股基本面、估值、财报、SEC、机构、资金流、宏观、产业链和风险研究 | 固定展示最后发布的 NVDA 研究快照；研究操作禁用 |
| 实验室 | 策略回测、事件研究、筛选、信号候选与运行记录 | 仅展示已发布结果；运行与修改功能禁用 |
| 花费 | 查看模型、Tavily 和 Financial Datasets 的调用量及估算费用 | 仅展示最后发布的费用快照；刷新功能禁用 |

在“盯盘”页面点击持仓行，可打开该股票的价格走势图或 K 线图，并切换 1 周、1 月、3 月、6 月、1 年和 3 年周期。访客看到的是服务器预先生成的冻结行情快照；两种图表共用同一份 OHLCV 数据，因此切换“走势/K线”不会产生新的网络请求。

所有者点击“刷新数据”后，页面数据会立即更新，同时后台依次为全部当前持仓生成上述六个周期的公开图表快照。页面会显示生成进度。任务采用单队列顺序执行，同一时间不会重复启动第二个批量任务；某个标的失败也不会阻止其他标的发布。访客的“盘前盘后”开关被禁用，以确保访客始终只读取已发布的正常交易时段快照。

> [!NOTE]
> 演示站点中的持仓来自 Paper Account，仅供项目效果展示。访客模式是服务器端强制只读，不只是把按钮隐藏起来。
...
> 当前状态：**持续调整中，正式测评尚未完成。** 设计与运行说明见 [`v2/agent_v2/README.md`](v2/agent_v2/README.md)。
...
> 当前状态：**持续调整中，正式测评尚未完成。** 设计与运行说明见 [`v2/agent_v3/README.md`](v2/agent_v3/README.md)。
...
当前表格描述的是设计目标和代码架构差异，不是测评结论。仓库暂未提供在相同实验条件下得到的 V2/V3 正式质量排名、准确率、成本、延迟或稳定性对比报告。
...
所有者在网页中刷新或打开允许公开的页面时，前端会把响应保存为公开快照。手动刷新盯盘数据还会在后台为全部当前持仓发布 1 周、1 月、3 月、6 月、1 年和 3 年价格历史快照。盯盘可展示 Paper Account 持仓；研究页的公开快照限定为 NVDA；实验室和花费页只展示已发布结果。访客不能刷新、提交表单、调用智能体或修改监控配置。后端同时设有全局访问拦截，因此隐藏按钮并不是唯一安全措施。
...
统一的 V2 / V3 评测题集与运行器在 [`v2/agent_bench/`](v2/agent_bench/README.md)：84 道开发题加 20 道 holdout 题，每题带评分细则，可录制并回放冻结的工具数据，由同一裁判打分并做成对盲评。它产出的是待人工复核的运行记录，不是已发布的测评结论。
