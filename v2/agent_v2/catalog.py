...
_RESEARCH_GUIDANCE = "stock_research：围绕公司的核心投资矛盾组织答案，不逐项报分。ROIC、ROE、利润率等比率只有在超过 100% 时才提示其依赖数据与计算口径，且只点名超过的那一个，不把没超过的数字说成超过。问风险时分三层写：公司自身（经营、财务、申报里的风险因素）、行业（竞争、周期、供应链）、宏观（利率、政策、汇率），每层至少一条有证据的，没有证据的那层明说。"
